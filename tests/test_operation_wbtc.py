...
def test_wbtc_1(currency,Strategy, strategy_wbtc_hbtc,curvePool, accounts, hCRV,yvaultv2, orb,rewards,chain,yhbtcstrategyv2,live_wbtc_vault, ychad, whale,gov,strategist, interface):
    yvault = yvaultv2
...
def test_wbtc_migrate(currency,Strategy, Contract, wbtcstrategynew, live_strategy_wbtc,curvePool, accounts, hCRV,yvaultv2, orb,rewards,chain,yhbtcstrategyv2,live_wbtc_vault, ychad, whale,gov,strategist, interface):
    yvault = yvaultv2
    vault = live_wbtc_vault
    currency = interface.ERC20(vault.token())
    gov = accounts.at(vault.governance(), force=True)
...
#obtc
def test_wbtc_2(currency,Strategy, strategy_wbtc_obtc, accounts,curvePoolObtc, obCRV, yvaultv2Obtc, orb,rewards,chain,yhbtcstrategyv2,live_wbtc_vault, ychad, whale,gov,strategist, interface):
    yvault = yvaultv2Obtc
    vault = live_wbtc_vault
    currency = interface.ERC20(vault.token())
    gov = accounts.at(vault.governance(), force=True)
