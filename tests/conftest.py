...
@pytest.fixture
def gusd(interface):
    yield interface.ERC20("0x056Fd409E1d7A124BD7017459dFEa2F387b6d5Cd")
...
@pytest.fixture
def yvaultv2Gusd(interface):
    yield interface.IVaultV2("0x2a38B9B0201Ca39B17B460eD2f11e4929559071E")
...
@pytest.fixture
def gusd_vault(pm, gov, rewards, guardian, gusd):
    currency = gusd
    Vault = pm(config["dependencies"][0]).Vault
    vault = gov.deploy(Vault)
    vault.initialize(currency, gov, rewards, "", "", guardian, {"from": gov})
...
@pytest.fixture
def curvePoolGusd(interface):
    yield interface.ICurveFi("0x4f062658EaAF2C1ccf8C8e36D6824CDf41167956")


@pytest.fixture
def curveTokenGusd(interface):
    yield interface.ICrvV3("0xD2967f45c4f384DEEa880F807Be904762a3DeA07")
...
@pytest.fixture
def strategy_dai_usdn(
    strategist, keeper, dai_vault, Strategy, depositUsdn, usdn3crv, usdnyvault, threecrv
):
    strategy = strategist.deploy(
        Strategy,
        dai_vault,
...
def strategy_gusd_gusd(
    ...
    strategy = gov.deploy(
        ...
        curvePoolGusd,  # curve pool
        curveTokenGusd,  # curve token
        yvaultv2Gusd,  # yVault
        2,  # pool size
        zeroaddress,  # meta token
        False,  # has underlying
        "ssc gusd gusd",  # strategy name
    )
    strategy.setHealthCheck(healthcheck)
    strategy.setKeeper(keeper)
    yield strategy
...
@pytest.fixture
def live_strategy_wbtc_bbtc(
    gov,
    keeper,
    live_strategy_wbtc_obt,
    live_wbtc_vault,
    healthcheck,
    Strategy,
    curvePoolBbtc,
    ...
    strategist = gov
    tx = live_strategy_wbtc_obt.cloneSingleSidedCurve(
        live_wbtc_vault,
        strategist,
        60 * 1e8,
        360,
...
@pytest.fixture
def strategy_hbtc_hbtc(
    gov, keeper, hbtc_vault, Strategy, curvePool, hCRV, yvaultv2, zeroaddress
):
    strategy = gov.deploy(
        Strategy,
        hbtc_vault,
